...
export default function Quizes() {
  ...
  const fetchQuizesInCourse = async (courseID) => {
    ...
    const quizAttempts = await quizClient.fetchAllQuizAttempts(currentUser._id);
    ...
    <div id="wd-assignments">
      ...
      <ul className="list-group quiz-cards " id="wd-quiz-list">
        {quizes.map((quiz: any, index: number) => {
          ...
          // const thisQuizAttemptScore = quizAttempts.find(q=> q._id == quiz._id);
          const thisQuizAttemptScore = quizAttempts.find(q => q.quizId === quiz._id);
            ...
            <li key={index} className="wd-quiz-list-item list-group-item quiz-card p-4">
              ...
              <div className="card-body d-inline ml-5">
                ...
                <p>
                  ...
                  {quiz.questionData?.length || 0} Questions | Score : 
                  {thisQuizAttemptScore?.totalScore }
                </p>
              </div>
              ...
              <div className="card-right">
                {isFaculty ? (
                  <div className="quiz-control">
                    <button className="btn" onClick={() => updatePublish(quiz)}>
                    {quiz.published ? (
                      ...
                      <ul className="dropdown-menu z-10 " aria-labelledby="quizContextMenuButton">
                        <li>
                          <button className="dropdown-item">
                            <Link className="wd-quiz-link nav-link " to={`/Courses/${cid}/Quizes/${quiz._id}/`}>
                              Edit
                            </Link>
                          </button>
                        </li>
